...
describe("Product State Derivation", () => {
  ...
  it("exposes the running state while transformation is active", () => {
    expect(deriveProductState(makeState({ phase: "running" })).state).toBe(
      "running",
    );
  });
  ...
  it("Terminal incomplete runs without a generated files index stay incomplete", () => {
    ...
    expect(result.message).toContain(
      "Required generated artifacts are unavailable",
    );
  });
  ...
  it("treats missing generated artifacts after terminal completion as pending", () => {
    expect(deriveProductState(makeState({ phase: "completed" })).state).toBe(
      "generated-pending",
    );
  });
  ...
  it("surfaces an otherwise generic failed run when no more specific artifact state exists", () => {
    expect(
      deriveProductState(makeState({ phase: "failed", error: "run failed" }))
        .state,
    ).toBe("failed");
  });
  ...
  describe("W0.2 workflow-driven states", () => {
    it("phase=starting without workflow maps to submitting", () => {
      expect(deriveProductState(makeState({ phase: "starting" })).state).toBe(
        "submitting",
      );
    });
    ...
    it("refuses to claim success when finalClassification is absent despite artifact agreement", () => {
      ...
      expect(result.message).toContain(
        "BFF final classification is unavailable",
      );
    });
