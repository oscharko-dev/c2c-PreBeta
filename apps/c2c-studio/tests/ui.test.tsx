...
describe("UI Primitives", () => {
  describe("Badge", () => {
    it("covers the variant matrix", () => {
      ...
      expect(screen.getByText("Incomplete").parentElement).toHaveClass(
        "bg-violet-soft",
      );
  ...
  describe("IconButton", () => {
    it("renders correctly and handles clicks", () => {
      const onClick = vi.fn();
      render(
        <IconButton icon={Search} onClick={onClick} aria-label="Search" />,
      );
    ...
    it("covers active and semantic variants", () => {
      ...
      expect(
        screen.getByRole("button", { name: "Default active" }),
      ).toHaveClass("bg-bg-3");
  ...
  describe("TreeRow", () => {
    ...
    it("supports keyboard activation and preserves long labels via title", () => {
      ...
      expect(
        screen.getByTitle(
          "target/java/com/c2c/w0/targetJava/transformation-metadata.json",
        ),
      ).toBeInTheDocument();
    });
  });
    ...
    it("truncates long path-like and hash-like strings while preserving the full title", () => {
      render(
        <div>
          ...
          />
          <Truncate
            text="cb4ddf023b0f57e79d63de91205f4fe31f6d6bf0"
            maxLength={14}
            position="middle"
          />
        </div>,
      ...
      ).toHaveTextContent("target/ja...data.json");
      expect(
        screen.getByTitle("cb4ddf023b0f57e79d63de91205f4fe31f6d6bf0"),
      ).toHaveTextContent("cb4ddf0...f6d6bf0");
    });
  });
        ...
        <div>
          ...
          <MetadataRow
            items={[
              {
                label: "source",
                value: "corpus/synthetic/branch-account-guard.cbl",
                truncate: "middle",
              },
              {
                label: "hash",
          ...
          <StatusBar
            ...
            items={[
              { label: "build", value: "match", valueVariant: "success" },
            ]}
          />
        </div>,
      );
