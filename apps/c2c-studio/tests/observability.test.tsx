...
function makeMinimalState(overrides: Partial<TransformationRunState>): TransformationRunState {
  return {
    phase: 'idle',
    runId: null,
...
function makeContextValue(stateOverrides: Partial<TransformationRunState>): TransformationRunContextValue {
  ...
  it('renders Experience Learning unavailable state', () => {
    vi.mocked(transformationRun.useTransformationRun).mockReturnValue(
      makeContextValue({
        phase: 'completed',
        runId: 'r1',
        experience: { runId: 'r1', programId: 'p1', mode: 'live', productMode: 'unavailable' },
      })
    );
