...
export function useGeneratedArtifacts() {
  ...
  if (!context) {
    throw new Error(
      "useGeneratedArtifacts must be used within a GeneratedArtifactsProvider",
    );
  }
  return context;
}
...
const GeneratedArtifactsContext = createContext<GeneratedArtifactsValue | null>(
  null,
);
...
function useGeneratedArtifactsState(): GeneratedArtifactsValue {
  ...
  const [unavailableFiles, setUnavailableFiles] = useState<Set<string>>(
    new Set(),
  );
  ...
  const artifactSource =
    showingHistoricalArtifacts && state.previousRun ? state.previousRun : state;
  ...
  const artifactState: GeneratedArtifactState = useMemo(() => {
    ...
    if (!showingHistoricalArtifacts && state.phase === "incomplete")
      return "incomplete";
    if (
      !showingHistoricalArtifacts &&
    ...
    if (buildTest && evidence) {
      if (
        ...
        generated.artifactRef?.sha256 ===
          buildTest.generatedArtifactRef?.sha256 &&
  ...
  const selectedFileRef = useMemo(() => {
    ...
    return (
      generatedFiles?.files.find((file) => file.path === selectedFilePath) ??
      null
    );
  ...
  const artifactDetails: ArtifactDetails | null = useMemo(() => {
    ...
    return {
      ...
      missingArtifacts:
        generated.missingArtifacts || generatedFiles?.missingArtifacts,
  ...
  useEffect(() => {
    ...
    async function fetchFile() {
      ...
      const result = await apiClient.getGeneratedFile(
        requestedRunId,
        selectedFilePath,
      );
...
export function GeneratedArtifactsProvider({
  children,
}: {
  children: ReactNode;
}) {
