...
export function BottomWorkbench() {
  ...
  const activeTab =
    bottomWorkbenchTabs.find((tab) => tab.id === activeBottomTab) ??
    bottomWorkbenchTabs[0];
    ...
    >
      ...
      <div className="flex min-h-10 items-center justify-between gap-2 border-b border-line-2 px-2 py-1 shrink-0 bg-bg-2">
        <Tabs
          ...
          tabs={bottomWorkbenchTabs.map((tab) => ({
            value: tab.id,
            label: tab.label,
          }))}
      ...
      >
        {activeTab.id === "run" && (
          <RunLifecyclePanel emptyState={activeTab.emptyState} />
        )}
        {activeTab.id === "agent" && (
          <AgentActivityPanel emptyState={activeTab.emptyState} />
        )}
        {activeTab.id === "build-test" && (
          <BuildTestPanel emptyState={activeTab.emptyState} />
        )}
        {activeTab.id === "artifacts" &&
          (state.phase === "idle" ? (
            <div className="p-4 space-y-2 text-sm">
              <p className="font-medium text-text">
                {activeTab.emptyState.title}
              </p>
        ...
        {activeTab.id === "evidence" && (
          <EvidencePackPanel emptyState={activeTab.emptyState} />
        )}
        {activeTab.id === "problems" && (
          <ProblemsPanel emptyState={activeTab.emptyState} />
        )}
