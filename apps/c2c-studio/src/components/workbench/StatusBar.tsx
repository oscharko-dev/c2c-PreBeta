...
import { useLineageCoverage } from "../../stores/lineageCoverage";
...
function runLabelFromProductState(
...
export function StatusBar({ apiState }: StatusBarProps) {
  ...
  // Studio-IDE-6 (#248): lineage coverage for the active generated Java
  // file. `null` means no file is selected (or no overlay has landed yet).
  const lineageCoverage = useLineageCoverage();

  const runLabel = runLabelFromProductState(
    productState.state,
    state.programId,
  );
  ...
  return (
    <footer
      className="flex min-h-6 flex-wrap items-center justify-between gap-x-4 gap-y-1 border-t border-line bg-accent-dim px-3 py-1 text-xs text-text-bright shrink-0"
    ...
    >
      ...
      <div className="flex min-w-0 items-center gap-4">
        ...
        <span
          data-testid="status-bar-lineage-coverage"
          className="text-text-bright/80"
          title="Share of the active generated Java file that has lineage back to COBOL; manual regions count as non-covered"
        >
          Lineage: {lineageCoverage === null ? "—" : `${lineageCoverage.pct}%`}
        </span>
        <span>UTF-8</span>
        {readiness.tone === "loading" ? (
          ...
          <span className="flex items-center gap-1">
            <span className="h-2 w-2 rounded-full bg-success"></span> Ready
          </span>
        ) : readiness.tone === "warning" ? (
          <span className="flex items-center gap-1">
            <span className="h-2 w-2 rounded-full bg-warn"></span>{" "}
            {readiness.statusBarLabel}
          </span>
        ) : (
          <span className="flex items-center gap-1">
            <span className="h-2 w-2 rounded-full bg-error"></span> Blocked
          </span>
        )}
        {!loading && error ? <span className="truncate">{error}</span> : null}
      </div>
    </footer>
  );
}
