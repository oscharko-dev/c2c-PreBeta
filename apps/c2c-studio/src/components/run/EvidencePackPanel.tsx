...
export function EvidencePackPanel({
  ...
  const ev =
    state.evidence ??
    (showingHistoricalEvidence ? (state.previousRun?.evidence ?? null) : null);
  if (!ev) {
    return (
      <div className="p-4 space-y-2 text-sm">
  ...
  const previousEvidence =
    state.evidence && state.previousRun?.evidence
      ? state.previousRun.evidence
      : null;
        ...
        <div className="space-y-4">
          <div className="bg-bg-1 border border-line-2 rounded p-4">
            ...
            <div className="grid grid-cols-1 gap-2 font-mono text-xs">
              ...
              <div className="flex flex-col">
                <span className="text-text-dim font-sans mb-1">
                  Manifest SHA256
                </span>
                <span className="text-text break-all">
                  {ev.manifestHash || ev.artifactRef?.sha256 || "N/A"}
                </span>
              </div>
              <div className="flex flex-col">
                <span className="text-text-dim font-sans mb-1">
                ...
                <span className="text-text break-all">
                  {ev.generatedArtifactRef?.sha256 || "N/A"}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-bg-1 border border-line-2 rounded p-4">
            ...
            <div
              className={`mb-3 text-xs ${intentionalDivergence ? "text-warn" : alignment.aligned ? "text-success" : "text-error"}`}
            >
              {intentionalDivergence
            ...
            <div className="space-y-3 font-mono text-xs">
              {alignment.entries.map((entry) => (
                <div
                  key={entry.label}
                  className="rounded border border-line-2 p-3"
                >
                  <div className="mb-1 font-sans text-text-dim">
                    {entry.label}
                  </div>
                  <div className="break-all text-text">
                    {entry.ref?.sha256 || "No hash available"}
                  </div>
                  <div className="mt-1 text-text-faint">
                    {entry.ref?.kind || "Content-addressed reference"}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="rounded border border-line-2 bg-bg-1 p-4">
              ...
              <div className="space-y-2 font-mono text-xs text-text">
                ...
                <div className="break-all">
                  {previousEvidence.manifestHash ||
                    previousEvidence.artifactRef?.sha256 ||
                    "N/A"}
                </div>
                <div className="break-all">
