...
const cobolWithPrevious: CobolHistoryEntry = {
  previous: {
...
describe("DiffWorkspace", () => {
    ...
    it("renders the no-previous-run notice when javaHistory is undefined", () => {
      ...
      render(
        <DiffWorkspace
          ...
          cobolHistory={cobolWithPrevious}
          onClose={onClose}
        />,
      );
  ...
  describe("populated state with lineage", () => {
    ...
    it("renders Java and COBOL diff editors with the expected sides", async () => {
      ...
      expect(cobol?.original).toBe(cobolWithPrevious.previous!.content);
      expect(cobol?.value).toBe(cobolWithPrevious.current.content);
    });
