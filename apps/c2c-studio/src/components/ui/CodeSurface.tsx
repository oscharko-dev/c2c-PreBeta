...
export const CodeSurface = React.forwardRef<HTMLDivElement, CodeSurfaceProps>(
  (
    { className, label, lines, copyValue, copyLabel, emptyMessage, ...props },
    ref,
  ) => {
    ...
    return (
      <div
        ...
        aria-label={label}
        className={cn(
          "flex min-h-0 flex-col overflow-hidden bg-bg-0 font-mono text-[12px]",
          className,
        )}
        {...props}
      >
              ...
              lines.map((line, index) => (
                <div
                  ...
                  className={cn("h-[18px] rounded-none pl-1 leading-[18px]", {
                    "bg-white/[0.025] shadow-[inset_2px_0_0_var(--accent-dim)]":
                      line.active,
