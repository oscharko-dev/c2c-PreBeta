  ...
  if (state.events && state.events.events.length > 0) {
    return (
      <div className="flex flex-col p-4 space-y-4 overflow-y-auto">
        <h3 className="text-sm font-semibold text-neutral-200">Harness Event Timeline</h3>
        <div className="space-y-4 border-l border-neutral-700 ml-2 pl-4">
          {state.events.events.map((evt, idx) => (
            <div key={idx} className="flex flex-col text-sm relative">
              ...
              <span className="text-neutral-500 text-xs">{evt.createdAt ? new Date(evt.createdAt).toLocaleString() : 'Unknown time'}</span>
              ...
              {evt.message && <span className="text-neutral-400 mt-1">{evt.message}</span>}
            </div>
          ))}
        </div>
      </div>
