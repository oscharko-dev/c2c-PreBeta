...
export function ModelGatewayPanel() {
  ...
  const deterministicNoModel = state.modelGatewayHealth?.error?.includes(
    "deterministic W0 mode",
  );

  if (
    !state.modelGatewayHealth ||
    state.modelGatewayHealth.status === "unavailable"
  ) {
    return (
      <div className="p-4 text-sm text-text-dim">
        ...
        {deterministicNoModel ? (
          <p className="text-xs italic text-text-dim">
            Note: Deterministic W0 COBOL-to-Java runs do not require model
            invocations. No Foundry or LLM participation was required or
  ...
  return (
    <div className="flex flex-col p-4 space-y-4 text-sm text-text">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <span className="block text-xs text-text-dim uppercase tracking-wider mb-1">
            Provider Mode
          </span>
        ...
        <div>
          ...
          <span className="font-medium text-text">
            {ledgerEnabled ? "Enabled" : "Disabled"}
          </span>
        </div>
        <div>
          <span className="block text-xs text-text-dim uppercase tracking-wider mb-1">
          ...
          <span className="font-medium text-text">
            {eventEmission ? "Enabled" : "Disabled"}
          </span>
        </div>
        <div>
          <span className="block text-xs text-text-dim uppercase tracking-wider mb-1">
        ...
        <div>
          ...
          <div className="space-y-2">
            {roleAvailability.map((role) => {
              const isAvailable =
                role.status === "ok" && role.availableModels.length > 0;
              return (
                <div
                  key={role.role}
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span
                      className={isAvailable ? "text-success" : "text-warn"}
                    >
                  ...
                  <p className="mt-1 text-xs text-text-dim">
                    {isAvailable
                      ...
                      : role.reason ||
                        "No approved active model is available for this role."}
                  </p>
                </div>
              );
            })}
