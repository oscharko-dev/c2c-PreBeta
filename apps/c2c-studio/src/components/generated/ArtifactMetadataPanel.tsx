    ...
    <div className="flex flex-col gap-2 p-4 text-sm text-text border-b border-line-2 bg-bg-1">
      ...
      {details.entryClass && (
        <div className="flex justify-between gap-4">
          ...
          <span className="truncate" title={details.entryClass}>
            {details.entryClass}
          </span>
        </div>
      )}
      ...
      {details.sha256 && (
        <div className="flex justify-between gap-4">
          ...
          <span className="font-mono text-xs truncate" title={details.sha256}>
            {details.sha256.substring(0, 16)}...
          </span>
        </div>
      )}
      ...
      {details.missingArtifacts && details.missingArtifacts.length > 0 && (
        <div className="mt-2 text-error">
          ...
          <ul className="list-disc list-inside text-xs pl-4">
            {details.missingArtifacts.map((ma) => (
              <li key={ma}>{ma}</li>
            ))}
          </ul>
        </div>
      ...
      {details.unsupportedFeatures &&
        details.unsupportedFeatures.length > 0 && (
          <div className="mt-2 text-warn">
            <div className="text-xs font-medium uppercase tracking-wider mb-1">
              Unsupported Features
            </div>
            <ul className="list-disc list-inside text-xs pl-4">
              {details.unsupportedFeatures.map((uf) => (
                <li key={uf}>{uf}</li>
              ))}
            </ul>
          </div>
