...
const UI_ERROR_CODE_SET: ReadonlySet<W02UiErrorCode> = new Set(W02_UI_ERROR_CODES);
...
export function mapOrchestratorFailureCode(raw: unknown): W02UiErrorCode | null {
...
const DEFAULT_MESSAGES: Record<W02UiErrorCode, string> = {
  unsupported_cobol: 'COBOL source uses constructs outside the supported W0.2 subset.',
  parse_failed: 'COBOL source could not be parsed.',
  semantic_ir_failed: 'Semantic IR generation failed for this program.',
  model_gateway_unavailable: 'The Model Gateway is unavailable. Try again shortly.',
...
const STACK_TRACE_MARKERS = /(at\s+[A-Za-z0-9_.$]+\s*\([^)]*\)|^\s*File\s+".+",\s+line\s+\d+|Traceback\s+\(most\s+recent\s+call\s+last\):)/m;

export function sanitizeUpstreamMessage(raw: unknown, fallback: string): string {
...
export function mapUpstreamUnavailable(detail?: string): MappedFailure {
  return {
    ...
    message: sanitizeUpstreamMessage(detail, defaultMessageFor('service_unavailable')),
  };
}
